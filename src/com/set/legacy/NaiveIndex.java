...
public class NaiveIndex {
   ...
   public static void main(String[] args) {
      ...
      final Path currentWorkingPath = Paths.get("src").toAbsolutePath();

      index.indexDirectory(currentWorkingPath);
      // My Code ---- karthik vallamsetla
      Scanner scan = new Scanner(System.in);
      while (true) {
          String myLine = scan.nextLine().trim();
          if  (myLine.equalsIgnoreCase("quit")) {
        	  scan.close();
        	  System.out.println("Bye!");
        	  break;
          } else {
        	  int wordCol = Arrays.binarySearch(index.mTermArray, myLine);
        	  if (wordCol > -1) {
        		  for (int i=0; i<index.mFileArray.length; i++ ) {
            		  if (index.mIndex[i][wordCol] == true) {
            			  System.out.println(index.mFileArray[i]);
            		  }
            	  }
        	  }
      ...
      // End My Code ---- karthik vallamsetla
